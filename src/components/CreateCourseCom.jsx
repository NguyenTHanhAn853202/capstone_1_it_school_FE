...
function CreateCourseCom() {
    const { updateTitle, updateDescription, updatePrice, updateLevel, updateThumbnail,updateCategory } = useCourse((state) => ({
    ...
    const {
        ...
        level = '',
        thumbnail,
        categoryId = '6717c8290230f05f8632ade4',
    } = useCourse((state) => state.course);
            ...
            <div className="flex justify-between">
                <div className="w-[47%] space-y-4">
                    <div>
                        ...
                        <select onChange={(e) => updateCategory(e.target.value)} className="outline-none bg-ip_dark rounded-lg p-2 w-full shadow-shadow shadow-md">
                            <option value="">Chọn chuyên môn</option>
                            {category.map((item,index)=><option key={index} value={item._id}>{item.name}</option>)}
                        </select>
                    </div>
                    <UploadFileCard
                        accept=".png,.jpg,.jpeg"
